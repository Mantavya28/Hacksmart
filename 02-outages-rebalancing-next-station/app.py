                ...
                with st.expander(f"🔴 **{station_name}** - {len(periods)} Critical Period(s) ({total_hours} hours)", expanded=True):
                    ...
                    for i, period in enumerate(periods, 1):
                        ...
                        <div style="background-color: #FFE5E5; border-left: 4px solid #DC3545; padding: 10px; margin: 8px 0; border-radius: 4px;">
                            ...
                            <strong>⏰ Time Range:</strong> {start_time} - {end_time} (Day 7)<br>
    ...
    if 'stations' not in st.session_state:
        ...
        with col2:
            ...
            if 'optimization_result' in st.session_state and st.session_state.optimization_result:
                ...
                m1.metric("Optimal Latitude", f"{res['lat']:.6f}°")
                m2.metric("Optimal Longitude", f"{res['lon']:.6f}°")
                m3.metric("Demand Std Dev", f"{res['std_dev']:.4f}")
                ...
                for station_id, demand_idx in res['demand_indices'].items():
                    ...
                    # Get original demand index (N/A for new station)
                    original_demand = res.get('original_demand_indices', {}).get(station_id, None)
                    original_demand_str = "N/A" if is_new else (f"{original_demand:.4f}" if original_demand is not None else "N/A")
                    
                    demand_data.append({
                        "Station ID": station_id,
                        "Station Name": station_name,
                        "Original Demand Index": original_demand_str,
                        "New Demand Index": f"{demand_idx:.4f}",
                        "Type": "🆕 New" if is_new else "Existing"
                    })
                
                # Sort by new demand index descending
                demand_data_sorted = sorted(demand_data, key=lambda x: float(x["New Demand Index"]), reverse=True)
